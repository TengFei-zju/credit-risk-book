...
### 6.2.3 为什么树模型在表格数据上优于深度学习

在信贷风控等表格数据场景中，梯度提升树（GBDT）通常优于深度学习模型。这一现象背后有深刻的统计学原因。

**理论解释**（来自多项对比研究）：

1. **归纳偏置的差异**
   - 树模型：假设特征是独立的，通过分裂点选择捕捉非线性关系
   - 神经网络：假设特征之间存在平滑的线性或低维流形结构
   - 表格数据通常特征是人工设计的，彼此独立，符合树模型的归纳偏置

2. **样本效率**
   - 树模型：少量样本即可学习有效的分裂规则
   - 神经网络：需要大量样本才能收敛到良好解
   - 信贷数据通常只有几万到几十万样本，对神经网络来说偏少

3. **特征工程的作用**
   - 树模型：可以直接使用原始特征，对特征缩放不敏感
   - 神经网络：需要精心设计的特征工程和归一化
   - 风控场景已有成熟的特征工程体系（WOE、IV 等）

4. **可解释性要求**
   - 树模型：可以通过 SHAP、特征重要性解释
   - 神经网络：黑箱模型，难以满足监管要求

**实证研究结果**：
- Grinsztajn et al. (2022) 在 48 个表格数据集上对比，发现 XGBoost/LightGBM 平均优于神经网络
- 仅在超大数据集（>100 万样本）或高度结构化特征（如图像、文本）上，神经网络才有优势

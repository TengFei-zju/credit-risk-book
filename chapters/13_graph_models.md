...
在将图结构信息输入传统机器学习模型之前，需要将图转换为特征向量。以下是常用的图特征提取方法。

#### 13.3.1 节点级特征

**度数中心性**（Degree Centrality）：
- 定义：与节点直接相连的边的数量
- 风控含义：关联越多，可能是中间人或团伙核心
- 计算：(v) = deg(v)$

**接近中心性**（Closeness Centrality）：
- 定义：节点到图中所有其他节点的最短路径平均长度
- 风控含义：越接近中心，信息/风险传播越快
- 计算：(v) = rac{1}{\sum_{u
eq v} d(v,u)}$

**介数中心性**（Betweenness Centrality）：
- 定义：经过该节点的最短路径数量
- 风控含义：桥梁角色，可能是关键担保人或中介
- 计算：(v) = \sum_{s
eq v
...
- 定义：图中极大连通子图
- 风控含义：独立的连通分量可能是独立的欺诈团伙

#### 13.3.3 图嵌入方法

**DeepWalk**：
- 原理：在图上进行随机游走，将游走序列输入 Word2Vec
- 优点：捕捉节点的局部邻域结构
- 缺点：随机性大，不稳定

**node2vec**：
- 原理：改进的随机游走，平衡 BFS（局部）和 DFS（全局）
- 优点：可调节参数控制探索策略
- 缺点：需要调参
